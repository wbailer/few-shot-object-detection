...
def get_additional_annotations(dataset,imglist,classes,gt_annos,max_nr = None,min_nr = None, classifier_path=None, modelcfg=None, conf=0.1):
        ...
        while len(filtered_annos['annotations'])<min_nr:
            ...
            for img in newimgobjs:
                ...
                for a in annots['annotations']:
                    ...
                    print("searching "+str(catid)+" found "+str(a['category_id']))
            ...
            if len(filtered_annos['annotations'])<min_nr:
                allimgs = gt_annos.getImgIds()
                catimgIds = gt_annos.getImgIds(allimgs,catlist)
                newImgs = random.choices(catimgIds,k=10)  
...
def generate_annotations(dataset,ss_gt_file,max_nr = None,min_nr = None, single_classifier=True):
    ...
    get_additional_annotations(dataset,base_images,dataset.novel_classes,gt_anno_set,max_nr,min_nr,classifier_path_novel,modelcfg_novel,novelconf)
    
    get_additional_annotations(dataset,novel_images,dataset.base_classes,gt_anno_set,max_nr,min_nr,classifier_path_base,modelcfg_base,baseconf)
